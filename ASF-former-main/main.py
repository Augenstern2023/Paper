...
# All rights reserved.
"""
ASF-former training and evaluating script.
"""
import argparse
import time
import yaml
...
def main():
    ...
    if args.eval_checkpoint:  # evaluate the model
        ...
        '''
        weight = {"w_attn_depth_all": w_attn_depth_all, "w_conv_depth_all": w_conv_depth_all, "w_attn_class_all": w_attn_class_all, "w_conv_class_all": w_conv_class_all}
        path = os.path.join(os.environ['HOME'], 'T2T-ViT', 'weight.pkl')
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            '''
            w_attn_depth_all += w_attn_depth
            w_conv_depth_all += w_conv_depth
            for i in range(len(target)):
                idx = target[i].item()
                if idx not in class_freq:
                    class_freq[idx] = 1
                    w_attn_class_all[idx] = w_attn_class[i]
                    w_conv_class_all[idx] = w_conv_class[i]
                else:
                    class_freq[idx] += 1
                    w_attn_class_all[idx] += w_attn_class[i]
                    w_conv_class_all[idx] += w_conv_class[i]

    w_attn_depth_all /= (batch_idx + 1)
    w_conv_depth_all /= (batch_idx + 1)
    for i in range(len(class_freq)):
        w_attn_class_all[i] /= class_freq[i]
        w_conv_class_all[i] /= class_freq[i]
    '''
