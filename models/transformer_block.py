...
class Block_conv(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.conv = ConvBranch_HMCB(
            in_features=dim, hidden_features=dim, out_features=dim)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x):
        # 处理不同形状的输入张量
        if x.dim() == 4:  # 如果是 (B, C, H, W) 形状
            B, C, H, W = x.shape
            HW = H * W
        elif x.dim() == 3:  # 如果是 (B, HW, C) 形状
            B, HW, C = x.shape
        else:
        ...
        HW_tensor = torch.tensor(
            HW, dtype=torch.float32, device=x.device)  # 显式指定设备
        ...
        return x, torch.zeros(B, device=x.device), torch.zeros(B, device=x.device)
...
class SafeNorm1d(nn.Module):
    """安全的1D归一化层，当batch_size=1时跳过归一化"""
...
class SafeNorm2d(nn.Module):
    """安全的2D归一化层，当batch_size=1时跳过归一化"""
...
class ASF_C_Encoder(nn.Module):
    def __init__(self, dim, num_heads, expand_disabled=True, expand_ratio=1.5,
        ...
        self.attn = Attention(self.attn_dim, num_heads=num_heads, qkv_bias=qkv_bias,
                              qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        conv_hidden_dim = int(self.conv_dim * conv_ratio)
        self.conv_branch = ConvBranch_HMCB(
            in_features=self.conv_dim, hidden_features=conv_hidden_dim, out_features=self.attn_dim)
    ...
    def forward(self, x):
        ...
        B, HW, C = x_conv.shape
        HW_tensor = torch.tensor(HW, device=x_conv.device)
        H, W = calculate_dimensions(HW_tensor)
        x_conv = x_conv.transpose(1, 2).reshape(B, C, H, W)
        attn_out = self.attn(self.norm1(x_attn))
        x_attn = attn_out[0] if isinstance(
            attn_out, (tuple, list)) else attn_out
