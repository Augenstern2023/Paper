...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.in_dim //
...
class Token_transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = attn_out[0] if isinstance(attn_out, (tuple, list)) else attn_out
...
class Token_CNN(nn.Module):
    ...
    def __init__(self, dim, in_dim, num_heads, mlp_ratio=1., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.conv = ConvBranch_HMCB(
            in_features=dim, hidden_features=dim, out_features=in_dim)
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(in_dim)
        self.mlp = Mlp(in_features=in_dim, hidden_features=int(
    ...
    def forward(self, x):
        ...
        if x.dim() == 4:  # 如果是 (B, C, H, W) 形状
            ...
            B, HW, C = x.shape
            HW = torch.tensor(HW, device=x.device)
        else:
            raise ValueError(f"Unexpected input tensor shape: {x.shape}")

        H, W = calculate_dimensions(HW)
...
class ASF_R_Encoder(nn.Module):
    def __init__(self, dim, in_dim, num_heads, expand_disabled=True, expand_ratio=1.5,
        ...
        self.attn = Attention(dim=self.attn_dim, in_dim=in_dim, num_heads=num_heads,
                              qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        conv_hidden_dim = int(self.conv_dim * conv_ratio)
        # Here out_features != self.conv_dim
        self.conv_branch = ConvBranch_HMCB(
            in_features=self.conv_dim, hidden_features=conv_hidden_dim, out_features=in_dim)
        self.fuse = Fusion_adaptive(in_dim)
        # self.fc2 = nn.Linear(in_dim, dim)

        self.norm2 = norm_layer(in_dim)
        mlp_hidden_dim = int(in_dim * mlp_ratio)
        self.mlp = Mlp(in_features=in_dim, hidden_features=mlp_hidden_dim,
