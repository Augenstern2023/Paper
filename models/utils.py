...
def resize_pos_embed(posemb, posemb_new):  # example: 224:(14x14+1)-> 384: (24x24+1)
    ...
    posemb_grid = F.interpolate(
        posemb_grid, size=(gs_new, gs_new), mode='bicubic')
...
def load_21k_pretrained(checkpoint_path, model, use_ema=False, num_classes=1000, del_posemb=False):
    if checkpoint_path and os.path.isfile(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        state_dict_key = 'state_dict'
        if isinstance(checkpoint, dict):
        ...
        if state_dict_key and state_dict_key in checkpoint:
            ...
            state_dict = checkpoint
        _logger.info("Loaded {} from pretrained model '{}'".format(
            state_dict_key, checkpoint_path))
        print("Loaded {} from pretrained model '{}'".format(
            state_dict_key, checkpoint_path))
...
def get_terminal_size():
    try:
        # 尝试使用shutil获取终端大小（Python 3.3+的跨平台方法）
